                        ...
                        <Card key={item.id}>
                            <Card.Content style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div style={{display:"flex",alignItems:'center',gap:'10px'}}>
                                    <PlayArrowIcon style={{cursor:'pointer'}} onClick={() => setSong(item?.track.preview_url)}/>
                                    <Avatar src={item?.track.album.images[0].url} />
                                    <b>{item?.track.name}</b>
                                </div>
                                <span>
                                    {new Date(item?.track.duration_ms).toISOString().substr(15, 5)}
                                </span>
                            </Card.Content>
                        </Card>
                    ))
